## Matteo Zocchi

**Automation & Control Engineering · Embedded & Low-Level Systems**
Milan, Italy 🇮🇹

---

Automation Engineering student at **Politecnico di Milano**, building toward control systems / instrumentation (I&C) engineering. I work at the layer where software meets the physical world: PLCs, control loops, embedded firmware, with a personal bias toward **reliability and performance**.

Comfortable across the whole stack modern automation actually runs on: real-time control and embedded C/C++, plus the software/DevOps side (Python, Docker, CI/CD) that increasingly sits next to it.

### Focus
- **Industrial automation & control** — PLC / SCADA / HMI (CODESYS), control logic, safety interlocks
- **Embedded & real-time** — C/C++, FreeRTOS, ESP32 / STM32
- **Software & infrastructure** — Python, Docker, CI/CD, Linux

### Selected projects
- **[Tank Level Control (PLC + HMI)](https://github.com/teozocchi/tank-level-control)** — two-mode level control in CODESYS Structured Text: on-off with hysteresis (v1) and PID on a modulating valve (v2), with safety interlocks, acknowledgeable alarms and an ISA-101-style HMI.
- **[mh-stories-re](https://github.com/teozocchi/mh-stories-re)** — binary reverse engineering: decoded two undocumented MT Framework formats (encrypted archives, reflective parameter tables) with Ghidra + x64dbg; published Python toolchain. First working scale mod in the series.
- **[abacus-api](https://github.com/teozocchi/abacus-api)** — containerized financial reconciliation engine: Flask API + heuristic/backtracking solver, PyPy runtime, Kubernetes.

### Currently
Starting Automation Engineering @ PoliMI (2026) · sharpening control-systems fundamentals · shipping projects.

📫 [zocchi.matteo1@gmail.com](mailto:zocchi.matteo1@gmail.com) · [LinkedIn](https://linkedin.com/in/matteo-zocchi)
